...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /Users/aaronksaunders/local/bin/node "/usr/local/bin/titanium" build --platform iphone --sdk "3.5.0.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
