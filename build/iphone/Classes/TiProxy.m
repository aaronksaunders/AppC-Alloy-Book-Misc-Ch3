...
//Rare exceptions to indicate a bug in the _cross_platformtemplate code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_cross_platformtemplate = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_cross_platformtemplate]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.cross_platformtemplate.module"
