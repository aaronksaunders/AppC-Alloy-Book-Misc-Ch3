...
NSString * Cross_PlatformTemplate$ModuleRequireFormat = @"(function(exports){"
...
@implementation Cross_PlatformTemplateObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_cross_platformtemplate);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Cross_PlatformTemplate global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Cross_PlatformTemplate" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_cross_platformtemplate = [[Cross_PlatformTemplateObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_cross_platformtemplate];
	
	NSString *_cross_platformtemplateNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _cross_platformtemplateNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_cross_platformtemplate valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Cross_PlatformTemplate$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.cross_platformtemplate.kroll" 
